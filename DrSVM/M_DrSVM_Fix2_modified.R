...
# In the modified version, we change eps = 1e-8 and quite = T
DrSVM_Fix2 <- function(x,y,lambda2,eps=10^(-8),smallmove=1,max_steps,scale=F,quiet=T) {
	np = dim(x)
	n = np[1]
	p = np[2]
	maxvars = min(p,n-1)
	...
	if (n!=lengY){
		if (quiet == F) cat("Error:  number of rows for x and y do not match \n")
		return(NULL)
	}

	test = (abs(y) == rep(1,lengY))
	if (sum(test) != lengY){
	  if (quiet == F) cat("Error:  y should only contain 1 or -1 \n")
		return(NULL)
	}
	...
	if (lambda2 < 0){
	  if (quiet == F) cat("Error: lambda may not be negative \n")	
		return(NULL)
	}

	if (smallmove <= 0){
	        if (quiet == F) cat("Error: smallmove must be positive \n");
        	return(NULL)
    	}

	if ((scale!=TRUE) & (scale!=FALSE)){
	  if (quiet == F) cat("Error:  value for scale must be FALSE or TRUE \n");
        	return(NULL)
	}
	
	if (max_steps <=0){
	  if (quiet == F) cat("Error:  value for max_steps must be greater than 0 \n")
		return(NULL)
	}
	...
	if (sum(y==1)==sum(y==-1)){
		# balanced case
	  if (quiet == F) cat("Initialize (balanced case) ...")	
		g = svmL1L2_init_bal(x,y,lambda2,eps)
		if (is.null(g)){
		  if (quiet == F)  cat("initial function failed\n")
			return(NULL)
		}	
		size = dim(g$beta)
		step = size[1]
		...
		# unbalanced case
	  if (quiet == F) cat("Initialize (unbalanced case): calling for simplex() ...")
	...
	while ((step<max_steps)&&(lambda1>0)){
		...
		if (sum(indexE)>0){
			...
			# |e| = 0
		  if (quiet == F) cat("\t ------------------- |E|==0 at this step -------------------\n")
			beta0_der=0
		...
		if (sum((!indexE)&nonezero_deltayf)>0){
			...
			if (sum(newL)>0){
				...
				if (sum(moveLeft)!=sum(newL)){
				  if (quiet == F) cat("\t Warning: point",indn[(!moveLeft)&newL],"just came to Left, but is coming back to Elbow!\n")
			...
			if (sum(newR)>0){
				...
				if (sum(moveRight)!=sum(newR)){
				  if (quiet == F) cat("\t Warning: point",indn[(!moveRight)&newR],"just came to Right, but is coming back to Elbow!\n")
		...
		if (sum(actvar)<p){
			...
			if (sum(newDropVar)>0){
				...
				if (abs(dropTemp1)<eps){
				  ...
				  if (quiet == F) cat("Error: no dropTemp is near 0 \n")
					dropTemp = Inf
				}
				if (dropTemp<0){
					dropTemp = Inf
		...
		if (sum(seq==delta_lam)>1){
		  if (quiet == F) cat("two events tie abnormally. \n")
			step = step - 1
			break
		}

		if (delta_lam>lambda1){
		  if (quiet == F) cat("\t Stop: no further events can happen, lambda1 will decrease to 0\n")
			delta_lam = lambda1
		}
		...
		if (quiet == F) cat("\t Generalized Corr -",delta_lam,"to",lambda1)
		...
		if (delta_lam==delta_lam1){
			...
			if (sum(index_fromL)>0){
			  if (quiet == F) cat("\t point",indn[index_fromL],"reached Elbow from Left\n")
			...
			if (sum(index_fromR)>0){
			  if (quiet == F) cat("\t point",indn[index_fromR],"reached Elbow from Right\n")
			...
			if (sum(index_fromL|index_fromR)==0){
			  if (quiet == F) cat("\t numerical error 1\n")
			}
			
			if (sum(actvar)<(sum(indexE)-1)){
			  if (quiet == F) cat("Warning: |v| < |e| - 1 \n")
			}
			...
			move_left = valid_alpha_der > 0
			if (sum(nonezero_alphader)==0){
			  if (quiet == F) cat("Error: alpha_der == 0\n")
			}
			...
			if (sum(index_toL)>0){
			  if (quiet == F) cat("\t point",indn[index_toL],"left Elbow to the Left\n")
			...
			if (sum(index_toR)>0){
			  if (quiet == F) cat("\t point",indn[index_toR],"left Elbow to the Right\n")
			...
			if (sum(index_toL|index_toR)==0){
			  if (quiet == F) cat("\t numerical error 2\n")
				break
			}
			...
			if (sum(index_toZero)>0){
			  if (quiet == F) cat("\t predictor",indp[index_toZero],"left {V} \n")
			  ...
			  if (quiet == F) cat("\t numerical error 3\n")
				break
			}
			...
			if (sum(actvar)<p){
				...
				if (sum(index_toV)>0){
				  if (quiet == F) cat("\t predictor",indp[index_toV],"joined {V}\n")
					actvar[index_toV] = TRUE
					actsign[index_toV] = -sign(cor_val[index_toV])
				}else{
				  if (quiet == F) cat("\t numerical error 4\n")
					break
				}
		  ...
		  if (quiet == F) cat("error: delta_lam has abnormal value \n")
			break
		}
		...
		if (loss[step]<eps){
		  if (quiet == F) cat("\t Stop: loss is 0 now!\n")
			break
		}else if (max_cor<eps){
		  if (quiet == F) cat("\t Stop: all correlations reach 0 now!\n")
			break
		}

	}	# while
	if (step == max_steps){
	  if (quiet == F) cat("\t Stop: max_steps is reached now!\n")
	}
	Cvec = cor_val
	cor = correlation
...
### y \in {1,-1}
	...
	p = np[2]
	if (sum(y==1)!=sum(y==-1)){
	  if (quiet == F) cat("Error: y is not balanced \n")
		return(NULL)
	}
	step = 1
	...
	if (sum(jstar)>1){
	  if (quiet == F) cat("Error: more than one predictor enters abnormally at 1\n")
		return(NULL)
	}
	...
	if (diff <=0){
	  if (quiet == F) cat("Abnormal data: cx(i_plus) < cx(i_neg) \n")
		return(NULL)
	}
	...
	if (sum(jstar2)>1){
	  if (quiet == F) cat("more than one predictor enters abnormally at 2\n")
		return(NULL)
	}
	...
	while (delta_lambda2 < delta_lambda){ # new predictor joint {v} before beta0 is fixed
		...
		if (delta_lambda == 0){
		  if (quiet == F) cat("Error: delta_lambda = 0 \n")
			return(NULL)
		}
		...
		if (sum(jstar>1)){
		  if (quiet == F) cat("more than one predictor enters abnormally at",step+1,"\n")
			return(NULL)
		}
		lam_sec = max(abs(temp_cor))
	...
	if (delta_lambda2 == delta_lambda){
	  if (quiet == F) cat("delta_lambda ties delta_lambda2 abnormally \n");
		return(NULL)
	}
	...
	if (abs(beta0[step,1]-beta0[step,2]) > eps){
	  if (quiet == F) cat("numerical error: beta0 is not fixed \n")
	}
	...
	if (sum(actvar)>1){
	  ...
	  if (quiet == F) cat("error: no actvar \n")
		return(NULL)
	}
	residuals = 1- y*fit
	...
	if (length(indexL) != length(y)){
	  if (quiet == F) cat("initial error: all points should be on L \n")
		return(NULL)
	}
	indexE = !indexL
...
svmL1L2_init_unbal <- function(x, y, lambda2, eps=10^(-9), smallmove = 1) {
	...
	if (sum(y==1) > sum(y==-1)){
		...
		# sum(y==1) == sum(y==-1)
	  if (quiet == F) cat("Error: y is balanced \n")
		return(NULL)
	}
	...
	if (g$solved == -1){
	  if (quiet == F) cat("simplex() can not converge to a solution \n")
		return(NULL)
	}else if (g$solved == 0){
	  if (quiet == F) cat("Warning: simplex() reaches the maximum iterations \n")
		return(NULL)
	}
	...
	if (sum(indexE)!= sum(actvar)){
	  if (quiet == F) cat("Elbow points =", sum(indexE),", Num of predictors =",sum(actvar),"\n")
		if ((sum(indexE)==0) && (sum(actvar)==1)){
			minVal = min(abs(res[index_bigclass]))
			nearestPnt = (abs(res) == minVal)&index_bigclass
			if (quiet == F) cat("Warning: there is no unique solution for beta0, we force the nearest point to stay on Elbow. \n")
			
			if (sum(nearestPnt)>1){	# two points have the same abs residuals
			  if (quiet == F) cat("Warning: Two points have the same absolute residual value\n")
				temp = res + runif(1)*eps # shift by a random value
				minVal = min(abs(temp[index_bigclass]))
				nearestPnt = (abs(temp)==minVal)&index_bigclass
				if (sum(nearestPnt) > 1){
				  if (quiet == F) cat("Warning: Two points have exactly the same residual value\n")
					return(NULL)
				}
			}			
			indexE[nearestPnt] = 1
		  ...
		  if (quiet == F) cat("Warning: simplex() does not converge to a correct minimum, try other smallmove value \n")
			return(NULL)
		}
	}
	...
	if (tmpqr$rank < length(b)){
	  if (quiet == F) cat("'A is singular, alphas can not be calculated \n")
		return(NULL)
	}else{
		sol = qr.solve(tmpqr,b)
	}
	...
	if (abs(sum(alpha * y)) > eps){
	  if (quiet == F) cat("error: sum(alpha_i * y_i) is not 0\n")
		return(NULL)
	}
	if (sum(alpha<0)>0){
		if (sum(0 - alpha[alpha<0]) > eps){
		  if (quiet == F) cat("error: negative alpha exists\n")	
			return(NULL)
		}else{
			alpha[alpha<0]=0
		}
	...
	if (sum(alpha>1)>0){
		if (sum(alpha[alpha>1]-1)>eps){
		  if (quiet == F) cat("error: some alpha has value > 1\n")
			return(NULL)
		}else{
			alpha[alpha>1] = 1
		}
	...
	if (lam_init <= 0){
	  if (quiet == F) cat("error: lambda has negative initial value \n")
		return(NULL)
	}
