...
max(abs(coef(m1, type="coefficients") -coef(m2, type="coefficients")))
...
#########################################################################
# DrSVM_Fix2 predictions
dyn.load("O_hsvmlassoNET.dll")

lambda2D = 1
n = nrow(dat$x)
md2 <- DrSVM_Fix2(x=dat$x, dat$y, lambda2D, eps=1e-10, 
                  smallmove=1,scale=F)
m01 = GCDpower(x=dat$x, y=dat$y, lambda = md2$lambda/n,
             lambda2=lambda2D/n, delta=0.01, method="hhsvm",eps=1e-10, maxit=3e7, standardize=F)

m012 = GCDpower(x=dat$x, y=dat$y, #lambda = md2$lambda/n,
               lambda2=lambda2D/n, delta=0.01, method="hhsvm",eps=1e-10, maxit=3e7, standardize=F)
plot(m012)

rownames(md2$beta)=NULL
rownames(m012$beta)=NULL
max(abs(t(md2$beta) - m01$beta))

source("O_utilities.R")
object = NULL
object$b0 <- apply(md2$beta0, 1, mean)
names(object$b0) <- paste("s", seq(object$b0), sep="")

object$beta <- t(md2$beta)
dimnames(object$beta) <- list(paste("V", seq(nrow(object$beta)), sep=""),
                           paste("s", (seq(ncol(object$beta))-1), sep=""))
object$lambda <- md2$lambda1
...
cc1 = M_coef.powerfamily(object, s = m012$lambda*n)
cc1 = M_coef.powerfamily(newobj(md2), s = m012$lambda*n)
cc2 = M_coef.powerfamily(m012, s = m012$lambda)
dimnames(cc1) = dimnames(cc2) = list(NULL, NULL)
...
m1.cv = cv.GCDpower(x=train_x, y=train_y, #lambda = md$lambda/n/delta,
                lambda2=1, qv=50, method="power",eps=1e-8, maxit=3e7, standardize=T)
m1.cv$lambda.min
