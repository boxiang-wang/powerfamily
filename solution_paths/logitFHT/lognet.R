lognet=function(x,is.sparse,ix,jx,y,weights,offset,alpha,nobs,nvars,jd,vp,ne,nx,nlam,flmin,ulam,thresh,isd,vnames,maxit,HessianExact,family){
  ...
  if(is.null(nc)){
    ...
    noo=nc[1]
    if(noo!=nobs)stop("x and y have different number of rows in call to logitnetFHT",call.=FALSE)
    nc=as.integer(nc[2])
    classnames=colnames(y)
  }
  if(family=="binomial"){
    if(nc>2)stop("More than two classes; use multinomial family instead in call to logitnetFHT",call.=FALSE)
...
### check if any are zero
    ...
    if(is.null(offset)){
      ...
      do=dim(offset)
      if(do[[1]]!=nobs)stop("offset should have the same number of values as observations in binomial/multinomial call to logitnetFHT",call.=FALSE)
      if((do[[2]]==1)&(nc==1))offset=cbind(-offset,offset)
      if((family=="multinomial")&(do[[2]]!=nc))stop("offset should have same shape as y in multinomial call to logitnetFHT",call.=FALSE)
    ...
    fit=if(is.sparse) .Fortran("splognet",
      parm=alpha,nobs,nvars,nc,x,ix,jx,y,offset,jd,vp,ne=ne,nx,nlam,flmin,ulam,thresh,isd,maxit,kopt,
                    ...
                    alm=double(nlam),
                    nlp=integer(1),
                    jerr=integer(1),PACKAGE="logitnetFHT"
                    )
  else .Fortran("lognet",
